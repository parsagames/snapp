...
class SnappBoxAccessibilityService : AccessibilityService() {
            ...
            object : TakeScreenshotCallback {
                override fun onSuccess(screenshot: ScreenshotResult) {
                    ...
                    try {
                        ...
                        mainHandler.post { speakOrder(amount, color, texts) }
                    } catch (_: Throwable) {
                        mainHandler.post { speakOrder(amount, OrderColor.UNKNOWN, texts) }
                    } finally {
    ...
    private fun speakOrder(amount: String?, color: OrderColor, texts: List<String>) {
        val safeAmount = amount ?: "نامشخص"
        val route = texts
            .filter { it.length in 3..100 }
            .filterNot { normalizedText(it).contains("قبول") || normalizedText(it).contains("پذیر") }
            .filterNot { it == safeAmount }
            .take(2)
        val routeText = if (route.isNotEmpty()) "، " + route.joinToString("، ") else ""
        val message = "سفارش جدید. رنگ دکمه ${color.persian}. مبلغ $safeAmount ریال$routeText. اگر می‌خواهی قبول شود بگو قبول کن."
